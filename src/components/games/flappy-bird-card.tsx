  ...
  return (
    <Card className="group flex flex-col h-full hover:scale-105 transition-all duration-300 border-blue-500 bg-gradient-to-br from-gray-900 to-gray-800 overflow-hidden">
      <div className="relative">
        <div className="h-48 bg-black border-b-2 border-blue-500 relative overflow-hidden flex items-center justify-center p-4">
        ...
        <CardHeader className="relative">
          <CardTitle className="text-2xl font-bold text-blue-400 group-hover:text-blue-300 transition-colors font-headline">
            Flappy Bird
          </CardTitle>
          <div className="flex items-center mt-1">
            {[...Array(3)].map((_, i) => <Star key={i} className="w-5 h-5 text-yellow-400 fill-current" />)}
            {[...Array(2)].map((_, i) => <Star key={i} className="w-5 h-5 text-gray-600 fill-current" />)}
          </div>
      ...
      <CardContent className="space-y-4 flex-grow flex flex-col justify-between">
        <div>
          <div className="flex flex-wrap gap-2 mb-4">
            <Badge className="bg-blue-600/20 text-blue-400 border-blue-500">Endless</Badge>
            <Badge className="bg-purple-600/20 text-purple-400 border-purple-500">Keyboard</Badge>
            <Badge className="bg-cyan-600/20 text-cyan-400 border-cyan-500">Touch</Badge>
        ...
        <div className="mt-auto">
          <Link href="/games/flappy-bird" className="block mt-4">
            <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 group-hover:shadow-lg transition-all font-headline">
